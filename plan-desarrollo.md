...
- [x] **T2 — Ranuras Q/E de loadout por estilo** (`combat-agent`, depende de T1). Zona se reasignó de Q a Mayús/Shift y Potenciador de E a Ctrl (mismos nombres de acción `zone_cast`/`potenciador` en `project.godot`, solo cambia la tecla física); Q y E pasan a ser 2 huecos de técnica equipable por estilo (`loadout_q`/`loadout_e`), cada uno con su técnica de fábrica (golpe en cono / estallido de área) definida en `style_data.gd` y con nombre propio por estilo en cada `.tres`. Punto de extensión para T4: `player.gd._equipped_loadout_technique()` resuelve la técnica activa de cada hueco por id (hoy solo `"factory"`) leyendo `NetworkManager.loadout_equipped` (Dictionary nuevo, sin escritor todavía) — T4 solo necesita añadir más ids al `match` de `submit_loadout_technique()`, no rehacer la ranura.
  ...
  **Fix de playtest 2026-09-03 (`combat-agent`)**: bug reportado como "la técnica gasta chakra pero no se percibe nada". Verificado con un test dirigido (instancia real de `player.tscn`+`enemy_simple.tscn` en modo headless, llamando a `submit_loadout_technique` directamente) que **la detección de golpe funciona bien** — Q (cono) y E (área) restan vida al enemigo correctamente, incluido el caso `shape=="area"` de E, que golpea a cualquier enemigo dentro del radio alrededor del jugador sin depender de `facing_dir`/`aim_point` **por diseño**, no por bug. El hueco real era exclusivamente de **feedback visual**: a diferencia de Proyectil/Zona, que instancian una escena visible aunque fallen, la técnica de loadout no dibujaba nada — el único indicio era `trigger_hit_shake()`, y solo si conectaba. Arreglado añadiendo `scripts/combat/melee_technique_fx.gd`/`scenes/combat/melee_technique_fx.tscn` (arco relleno para `shape=="cone"`, círculo relleno para `shape=="area"`, color placeholder por elemento igual que `status_tag.gd`/`zone_preview.gd`, se desvanece en ~0.22s): se instancia SIEMPRE en `confirm_loadout_technique` (golpee o no), centrado en el jugador que usa la técnica. No es una Zona (no aplica la regla de "siempre plano en pantalla", esa es exclusiva de `GroundZone`) — es un golpe puntual de melee, mismo criterio de espacio de mundo que `Projectile`/`StatusTag`. `confirm_loadout_technique` ganó 4 parámetros nuevos (`shape`, `facing_dir`, `shape_size`, `cone_degrees`) para poder dibujar el FX igual en todos los peers sin recalcularlo de forma potencialmente desincronizada. Cubre de paso los otros dos huecos pedidos en el reporte porque comparten la misma función: la técnica de fábrica sin coste de chakra de Físico y cualquier técnica del pool de pergaminos (T4, `style_data.pergaminos_pool`) — ninguno tiene lógica de daño/feedback propia, todos pasan por `submit_loadout_technique`/`confirm_loadout_technique`.
...
- [x] **T4 — Pool de técnicas de pergamino por estilo** (`combat-agent`, depende de T2). Cada estilo pasa de tener una única técnica Sellos comprable a tener varias técnicas aprendibles asignables a los huecos Q/E de T2. Catálogo compartido de 3 técnicas placeholder (`PergaminoTechnique`, `StyleData.pergaminos_pool`/`find_pergamino_technique()`) — mismo catálogo para los 6 estilos por ahora, dar sabor elemental propio queda para tuning/arte fuera de esta tanda. `NetworkManager.pergaminos_aprendidos` distingue "aprendida" (comprada, T5) de "equipada" (`loadout_equipped`, ya existía de T2). `player.gd.submit_equipar_tecnica_loadout`/`confirm_equipar_tecnica_loadout` es el punto de entrada host-autoritativo que T5 llamará tras cobrar fichas — no implementa la compra en sí.
- [x] **T5 — Flujo de compra con sustitución en la Tienda de Pergaminos** (`casino-agent`, depende de T4 commiteado). Comprar ya no desbloquea sin más: cada compra aprende la siguiente técnica del pool de `style_data.pergaminos_pool` que el jugador todavía no tenga para su estilo actual (mismo precio fijo de siempre, `tienda.precio_pergamino`, mismo mecanismo de fichas), y el jugador debe pulsar Q o E para decidir a qué hueco va (`player.gd._pergamino_pendiente_equipar`/`_confirmar_hueco_pergamino_pendiente`, se aborta solo si te alejas de la tienda sin elegir para no secuestrar esas teclas en combate). Esa elección llama directamente a `submit_equipar_tecnica_loadout` de T4 (no se reimplementa el equipado ni la sustitución): la técnica sustituida sigue en `NetworkManager.pergaminos_aprendidos`, reequipable gratis. Efecto secundario esperado (ya anticipado en la tabla de supuestos de arriba): esto retira la única vía para desbloquear la técnica de Sellos-en-R vieja (`NetworkManager.pergaminos_sellos_comprados`) — ese sistema (`style_data.gd` grupo "Sellos", `submit_sellos_technique`, y la trampa de `cartas_selladas.gd`) queda tal cual, sin tocar (fuera del scope de `casino-agent`, es T1-T4/combat-agent), pero ya no hay forma de comprarlo desde esta tienda.
- [x] **T6 — Auditoría de red del nuevo recurso** (`netcode-agent`). Cooldowns por ranura: seguros, no explotables — cada `submit_*` corre solo si `multiplayer.is_server()` y valida con `_validate_sender()` que quien pide la acción es el dueño del personaje, así que la decisión de cooldown/coste de chakra nunca se toma sobre datos que mande el cliente. Tick de sync (`CHAKRA_REGEN_SYNC_INTERVAL` = 0.5s) ya estaba implementado por T1, no hacía falta añadirlo. Hallazgo real: `confirm_chakra_sync` (único RPC nuevo de T1 que no nace de un `submit_*`) no validaba remitente — al ser `@rpc("any_peer", ...)`, un cliente modificado podía invocarlo directo sobre cualquier nodo `Player` y forjar chakra ilimitado en la copia autoritativa del host sin pasar por ningún `submit_*`. Corregido en `scripts/player/player.gd` (`confirm_chakra_sync`): el host ignora cualquier valor que le llegue (su propio tick es la fuente de verdad) y el resto de peers solo aceptan el valor si el remitente es el host (peer 1). Deuda documentada, fuera de este scope acotado: el resto de `confirm_*` de `player.gd` comparte el mismo patrón sin validar remitente (se confía en que solo el host los llama) — igual de falsificable en teoría, pendiente de una pasada de endurecimiento si el scope crece.
...
| `combat-agent` | Combate y estilos | T1-T4 del rework de combate de la sección 2.1 ya hechos (chakra pasivo + cooldowns, loadout Q/E, ranura de Soporte, pool de técnicas de pergamino equipables en Q/E). T5 (flujo de compra con sustitución) queda para `casino-agent`, T6 (auditoría de red) para `netcode-agent` |
